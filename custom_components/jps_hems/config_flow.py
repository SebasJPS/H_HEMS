...
def _csv_string(values: list[str] | None) -> str:
    """Convert entity ids to a comma separated string for forms."""
    return ", ".join(values or [])


class JpsHemsConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for JPS HEMS."""
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            data = {
                ...
                CONF_BATTERY_SOC_SENSORS: _csv(
                    user_input.get(CONF_BATTERY_SOC_SENSORS)
                ),
                CONF_BATTERY_DISCHARGE_SENSORS: _csv(
    ...
    def async_get_options_flow(
        ...
        return JpsHemsOptionsFlow(config_entry)

    def _schema(self, defaults: dict[str, Any]) -> vol.Schema:
        return _schema(defaults)


class JpsHemsOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(
        ...
        defaults = {
            ...
            CONF_PV_POWER_SENSORS: _csv_string(data.get(CONF_PV_POWER_SENSORS)),
            CONF_PV_AVERAGE_SENSOR: data.get(CONF_PV_AVERAGE_SENSOR),
            CONF_BATTERY_SOC_SENSORS: _csv_string(data.get(CONF_BATTERY_SOC_SENSORS)),
            CONF_BATTERY_DISCHARGE_SENSORS: _csv_string(
            ...
            CONF_FLEXIBLE_LOAD_SWITCHES: _csv_string(
                data.get(CONF_FLEXIBLE_LOAD_SWITCHES)
            ),
            CONF_WALLBOX_SWITCHES: _csv_string(data.get(CONF_WALLBOX_SWITCHES)),
            CONF_HEAT_PUMP_SWITCHES: _csv_string(data.get(CONF_HEAT_PUMP_SWITCHES)),
        }
